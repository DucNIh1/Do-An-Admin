...
export default function MonthlyTarget() {
  ...
  const target = 100;
  const progressPercentage = currentMonthData
    ? (currentMonthData.count / target) * 100
    : 0;
  ...
  const getComparisonIcon = (currentCount: number, previousCount: number) => {
      ...
      <div className="px-5 pt-5 bg-white shadow-default rounded-2xl pb-11 dark:bg-gray-900 sm:px-6 sm:pt-6">
        ...
        <div className="relative ">
          ...
          <span className="absolute left-1/2 top-full -translate-x-1/2 -translate-y-[95%] rounded-full bg-success-50 px-3 py-1 text-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">
            +{currentMonthData?.count || 0} học sinh
          </span>
        </div>
        <p className="mx-auto mt-10 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base">
          Trong tháng này bạn đã có **{currentMonthData?.count || 0}** học sinh
          đăng ký.
        </p>
      </div>
